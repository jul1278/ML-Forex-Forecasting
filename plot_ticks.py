...
def plot_tick_range(tick_path, range_start, range_end):
    ...
    for index, row in df.iterrows():
        rate_time = row['RateDateTime']

        rate_time_dt = parse_time(rate_time)

        if (rate_time_dt >= range_start) and (rate_time_dt <= range_end):
            append_row = (row['RateAsk'] + row['RateBid']) / 2.0
            ticks.append(append_row)

        if (rate_time_dt > range_end):
            break 
